...
function renderKPIs(){
  ...
  // leakage = zero-sale executives
  var zeroCount = 0;
  for(var i=0;i<EX.length;i++){ if(EX[i].jach===0) zeroCount++; }
  var leakPct = EX.length>0 ? Math.round((zeroCount/EX.length)*100) : 0;
...
function calcCOCA(){
  var paidCount=0;
  for(var i=0;i<EX.length;i++){ if(EX[i].jach>0) paidCount++; }
  var el=document.getElementById('branchCost');
  var cost=el?(parseFloat(el.value)||200000):200000;
  var coca=paidCount>0?Math.round(cost/paidCount):0;
  var txt=paidCount===0?'No sales':(coca>=1000?'₹'+(coca/1000).toFixed(0)+'K':'₹'+coca);
