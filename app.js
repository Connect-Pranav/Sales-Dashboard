...
function rebuildZFromOverrides() {
  ...
  // Single Source of Truth: update EVERY view after any heatmap edit
  renderKPIs();
  renderMgrDonuts();
  renderExecTable();
  renderLeaderboard();
  ...
  renderWeeklyBar();
  renderZDonuts();
  drawCircles(); // map bubble colors depend on achievement %
  ...
  _WEEKLY_CHART = new Chart(ctx, {
    ...
    options:{
      ...
      scales:{
        ...
        y:{grid:{color:'rgba(0,0,0,0.05)'},ticks:{font:{size:8},color:'#4a6080'},beginAtZero:true}
      }
    }
  });
}
