...
// ── KPIs ──────────────────────────────────────────────────────
function renderKPIs(){
  var tgt=0,ach=0,zc=0,ni=0,np=0;
  EX.forEach(function(e){tgt+=e.jtgt;ach+=e.jach;if(['Z2','Z3','Z4'].indexOf(e.zf)>=0)zc++;});
  var pct=tgt>0?((ach/tgt)*100).toFixed(1):0;
  // Leakage = Z-flagged execs / total (proper definition: data going cold)
  var zFlagged=EX.filter(function(e){return['Z2','Z3','Z4'].indexOf(e.zf)>=0;}).length;
  var leakPct=EX.length>0?((zFlagged/EX.length)*100).toFixed(0):0;
  var zeroExecs=EX.filter(function(e){return e.jach===0;}).length;
  ...
  setText('k-leak',leakPct>0?(leakPct+'%'):'0%');
  setText('k-lks',zFlagged>0?(zFlagged+' Z-flagged'):'No leakage ✓');
  calcCOCA();
}

function calcCOCA(){
  var costEl=document.getElementById('branchCost');
  var cost=costEl?Number(costEl.value)||200000:200000;
  var newPaid=EX.filter(function(e){return e.jach>0;}).length;
  var coca=newPaid>0?Math.round(cost/newPaid):0;
  var cocaDisplay;
  if(coca===0&&newPaid===0){
    cocaDisplay='No sales';
  } else if(coca>=100000){
    cocaDisplay='₹'+Math.round(coca/1000)+'K';
  } else if(coca>0){
    cocaDisplay='₹'+coca.toLocaleString();
  } else {
    cocaDisplay='₹0';
  }
  setText('k-coca',cocaDisplay);
  // also update sub
  setText('k-coca-s',newPaid>0?(newPaid+' paid clients'):'0 paid clients');
}
...
function openLeakPop(){
  var map=mgrMap();
  var zFlaggedList=EX.filter(function(e){return['Z2','Z3','Z4'].indexOf(e.zf)>=0;});
  var zeroExecs=EX.filter(function(e){return e.jach===0;});
  var lpct=EX.length>0?((zFlaggedList.length/EX.length)*100).toFixed(0):0;
  var html='<div style="text-align:center;margin-bottom:16px">'
    +'<div style="font-family:Syne,sans-serif;font-size:36px;font-weight:900;color:#e53e3e">'+lpct+'%</div>'
    +'<div style="font-size:12px;color:var(--t3)">Branch Leakage Rate (Z-flagged execs)</div>'
    +'<div style="font-size:11px;color:var(--t2);margin-top:6px;max-width:380px;margin-left:auto;margin-right:auto">'
    +(lpct>50?'🚨 Critical: Over half your executives are Z-flagged.'
    :lpct>20?'⚠ High leakage: Focus on Z2/Z3 coaching immediately.'
    :lpct>0?'⚠ Monitor: Some Z-flagged executives need attention.'
    :'✅ Zero leakage — All executives are active!')+'</div></div>'
    +'<div style="margin-bottom:12px"><div style="font-size:10px;font-weight:800;color:var(--t3);text-transform:uppercase;letter-spacing:1px;margin-bottom:8px">Z-Flagged Executives ('+zFlaggedList.length+')</div>'
    +(zFlaggedList.length===0?'<div style="text-align:center;padding:12px;color:#1a9e5c;font-weight:700">✅ No Z flags — No leakage!</div>'
    :zFlaggedList.map(function(e){
      return '<div class="lp-row"><span class="lp-name">'+e.name+'</span><span style="color:var(--t3);font-size:11px">'+e.mgr+'</span>'
        +'<span class="lp-val" style="color:#e53e3e">'+e.jach+'/'+e.jtgt+' · '+e.zf+'</span></div>';
    }).join(''))+'</div>'
...
function saveCell(){
  ...
  closePop('cellPop');
  renderHeatmap();
  // Rebuild exec Z flags with new data
...
function renderBI(){
  ...
  var cost=Number(document.getElementById('branchCost').value)||200000;
  var newPaid=EX.filter(function(e){return e.jach>0;}).length;
  var coca=newPaid>0?Math.round(cost/newPaid):0;
  ...
  if(coca>30000) bads.push({icon:'💸',val:'₹'+coca.toLocaleString(),lbl:'High COCA',desc:'Cost per new client is ₹'+coca.toLocaleString()+'. Target should be under ₹25,000. Optimize lead quality or reduce branch overhead.'});
  ...
  // INFOS
  infos.push({icon:'🧮',val:'₹'+coca.toLocaleString(),lbl:'COCA',desc:'Cost of Client Acquisition. Lower this by improving conversion rate from meetings.'});
  ...
  actions=[
    ...
    {pri:'💰 Cost',txt:'Reduce COCA from ₹'+coca.toLocaleString()+' by improving meeting-to-close rate. Current avg: '+((AREAS.reduce(function(a,x){return a+x.mp;},0)/AREAS.length).toFixed(1))+'%.'},
