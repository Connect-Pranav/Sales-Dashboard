...
function renderKPIs(){
  ...
  // Leakage: use NI col (12) + NP col (13) if available — approximate from exec data
  // Since we don't have NI/NP in EX objects, calculate from zero-sale weeks
  var zeroExecs=EX.filter(function(e){return e.jach===0;}).length;
  var leakPct=EX.length>0?((zeroExecs/EX.length)*100).toFixed(0):0;
  ...
  setText('k-lks',zeroExecs+' execs at 0 sale');
}
...
function calcCOCA(){
  ...
  setText('k-coca',coca>0?'₹'+coca.toLocaleString():'N/A');
}
...
function renderMgrDonuts(){
  ...
  MGR_DONUTS.forEach(function(name,idx){
    ...
    var cid='mdc_'+name.replace(/\s/g,'');
    var div=document.createElement('div');
    div.className='mdc';
    div.innerHTML='<div style="position:absolute;top:0;left:0;right:0;height:3px;background:'+c+'"></div>'
      +'<div class="mdc-name" onclick="openMgrPop(\''+name+'\')" style="color:var(--p1);text-decoration:underline;cursor:pointer">'+name+'</div>'
...
function openLeakPop(){
  ...
  var lpct=EX.length>0?((zeroExecs.length/EX.length)*100).toFixed(0):0;
  var html='<div style="text-align:center;margin-bottom:16px">'
    +'<div style="font-family:Syne,sans-serif;font-size:36px;font-weight:900;color:#e53e3e">'+lpct+'%</div>'
    +'<div style="font-size:12px;color:var(--t3)">Branch Leakage Rate</div>'
    +'<div style="font-size:11px;color:var(--t2);margin-top:6px;max-width:380px;margin-left:auto;margin-right:auto">'
    +(lpct>50?'🚨 Critical: Over half your executives have ZERO Jun sales. Immediate intervention required.'
    :lpct>30?'⚠ High leakage: Focus on coaching bottom performers and reviewing pitch quality.'
    :'✅ Acceptable leakage. Continue monitoring Z2 cases.')+'</div></div>'
    +'<div style="margin-bottom:12px"><div style="font-size:10px;font-weight:800;color:var(--t3);text-transform:uppercase;letter-spacing:1px;margin-bottom:8px">Zero Sale Executives</div>'
    +zeroExecs.map(function(e){
      return '<div class="lp-row"><span class="lp-name">'+e.name+'</span><span style="color:var(--t3);font-size:11px">'+e.mgr+'</span>'
        +'<span class="lp-val" style="color:#e53e3e">0/'+e.jtgt+' ('+e.zf||'—'+')</span></div>';
    }).join('')+'</div>'
